...
class ItemVarController extends Controller
{
    ...
    public function createAction(Request $request)
    {
        ...
        $event->setObjectType($this->objectType)
            ...
            ->setRequest($request)
            ->setFormAction($this->generateUrl('cart_admin_item_var_create'))
            ->setFormMethod('POST');

        $this->get('event_dispatcher')
            ->dispatch(CoreEvents::ITEM_VAR_ADMIN_FORM, $event);
        ...
        if ($event->isFormValid()) {
            ...
            $this->get('event_dispatcher')
                ->dispatch(CoreEvents::ITEM_VAR_CREATE_RETURN, $event);
    ...
    public function editAction(Request $request, $id)
    {
        ...
        $event->setObjectType($this->objectType)
            ->setEntity($entity)
            ->setRequest($request)
            ->setFormAction($this->generateUrl('cart_admin_item_var_update', ['id' => $entity->getId()]))
            ->setFormMethod('PUT');
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        if ($event->isFormValid()) {
            ...
            $this->get('event_dispatcher')
                ->dispatch(CoreEvents::ITEM_VAR_UPDATE_RETURN, $event);
        ...
        if ($ids) {
            foreach($ids as $id) {
                ...
                if ($event->getSuccess()) {
                    ...
                    $event->addSuccessMessage("{$counter} ItemVars deleted !");
                    $event->addErrorMessage("ItemVar ID: {$id} could not be deleted");
